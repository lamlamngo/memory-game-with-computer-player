...
public class PlayingDeck {
	 ...
	 *
	 * @param num The number of the desired card
	 ...
	 * @param num The number of cards to be selected
	 */
	...
	/**
	 * Updates the number of current cards when cards are removed
	 *
	 * @param num The number of cards removed
	 */
	public void decrementCurrent(int num){
		currentCards = currentCards - num;
	}
	...
	/**
	 * Returns the index of a selected card
	 *
	 * @param aCard The card that is selected
	 * @return The index of the selected card
	 */
	public int indexOf(Card aCard){
		if (cardsInPlayed.isEmpty()){
			return -1;
		}
		else{
			return cardsInPlayed.indexOf(aCard);
		}
	}

	/**
	 * new
	 * Checks if 2 cards have been selected
	 *
	 ...
	 *
	 * @param p The Graphics object
	 */
	public void display(Graphics p){
		int canvasWidth = PlayMemoryGame.getCanvasWidth();
		int canvasHeight = PlayMemoryGame.getCanvasHeight();
	...
	/**
	 * new
	 * Creates the deck of cards for Memory game
	 */
	public void createMemoryDeck(){
		cardsInDeck = new ArrayList<Card>(cardsInDeck.subList(0, 31));
		cardsInDeck.add(new Card(3,0,0,0));
		cardsInDeck.add(new Card(4,0,0,0));
		cardsInDeck.add(new Card(5,0,0,0));
		cardsInDeck.add(new Card(6,0,0,0));
		cardsInDeck.add(new Card(7,0,0,0));
		for (int i = 0; i < 36; i++){
	...
	public String toString(){
	    ...
	    for (int i = 0; i < currentCards; i++){
            returnSTR += " [" + i +"]" + cardsInPlayed.get(i).toString() + "\n";
        }
        return returnSTR;
    }
