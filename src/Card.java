 ...
 */
public class Card {
	private final int OUTLINEW = 80;
	private final int OUTLINEH = 80;
	...
	public Card(int shapeType, int color, int filling, int num){
		...
		myOutline = myFactory.createShape("Outline", 7, 0, 0);
	...
	/**
	 * Getter method that returns the number of shapes on the card
	 * 
	...
	/**
	 * Getter method that returns the filling of a card
	 * 
	...
	/**
	 * Getter method that returns the color of the shape(s) on the card
	 * 
	...
	/**
	 * new
	 * Checks if there are cards displayed on the game board
	 * 
	 * @return setDisplay True if cards are displayed and False otherwise
	 */
	...
	/**
	 * new
	 * Sets it so that card is not display on the game board
	 */
	...
	/**
	 * new
	 * Sets it so that the card is displayed on the game board
	 */
	...
	public void display(Graphics page, int outlineX, int outlineY){
		if (setDisplay){
			if (myOutline.isSelected()){
				if (myShape.getShape().equals("james") || myShape.getShape().equals("diego") || myShape.getShape().equals("varun") || myShape.getShape().equals("barr")||myShape.getShape().equals("Lam")){
					...
					myOutline.display(page);
					if (getNumber() == 1){
						spaceX = (OUTLINEW - shapeWith)/2;
						spaceY =  (OUTLINEH - shapeHeight)/2;
						myShape.setPosition(outlineX + spaceX, outlineY + spaceY);
						myShape.display(page);
					}
					else if (getNumber() == 2){
						spaceX = (OUTLINEW - shapeWith)/2;
						spaceY =  (OUTLINEH - shapeHeight)/6;
	...
	/**
	 * Displays the card and adjusts the sizes accordingly
	 * 
	 * @param page Graphics object
	 * @param outlineX x-coordinate of the top left corner of card outline
	 * @param outlineY y-coordinate of the top left corner of card outline
	...
	public void display1(Graphics page, int outlineX, int outlineY){
		...
		myOutline.setPosition(outlineX,outlineY);
		myOutline.display(page);
		//if the card only has one shape
		if (getNumber() == 1){
